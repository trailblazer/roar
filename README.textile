...
Representers are the key ingredients in Roar, so let's check them out!
...
Look how the @#from_json@ method helps extracting data from the incoming document and, again, @#to_json@ returns the freshly created order's representation. Roar's representers are truly working in both directions, rendering and parsing, thus preventing you from redundant knowledge sharing.
...
*Note:* If this looks clumsy - It's just the raw API for representers. You might be interested in the upcoming DSL that simplifies frequent workflows as updating a representer.
...
You can use <pre>Roar::Decorator</pre> as a representer. More docs coming soon. Note that if you want your represented object to save incoming hypermedia you need to do two things.
