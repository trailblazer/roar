...
class JsonHypermediaTest
  describe "Hypermedia API" do
    ...
    it "extracts links from JSON" do
      ...
      assert_equal(%w(self http://self), [@r.links.first.rel, @r.links.first.href])
    end

    it "extracts links from JSON with any attributes" do
      @r = @c.from_json({:links => [{:rel => "self", :href => "http://self", :template => "true", :blah => "123"}]}.to_json)

      assert_equal 1, @r.links.size
      assert_equal(%w(self http://self true 123), [@r.links.first.rel, @r.links.first.href, @r.links.first.template, @r.links.first.blah])
    end
