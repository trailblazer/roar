...
# The filled-out document is <code>POST</code>ed to the given URL.
...
Now that we got a fresh order, let's place some items! The system's API allows adding articles to an existing order by <code>POST</code>ing articles to a specific resource. This endpoint is propagated in the order document using *hypermedia*.
...
<pre>
beer = JSON::Article.new(:title => "Lonestar")
beer.post!(order.links[:items])
</pre>

That's all we need to do.

# First, we create an article representation.
...
The @order@ instance in the frontend is now stale since it doesn't contain items, yet.
...
Again, we use hypermedia to retrieve the order's URL.
