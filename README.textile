...
<pre>JSON::Article.new(
  :title  => "Beck's",
  :id     => 666).serialize # => "{\"article\":{\"id\":666, ...
</pre>

Using the @ModelRepresenting@ feature module we can take a shortcut.

<pre>JSON::Article.from_model(@article).
...
Rendering stuff is easy, representers define and serialize documents for us. However, representers can do more. They work _bi-directional_ in terms of rendering outgoing _and_ parsing incoming representation documents.

If we implement an endpoint for creating new orders, we'd allow POST to @http://orders/@. Let's explore the service code for parsing and creation.
...
The new representer abstraction layer seems complex and irritating first, where you used @params[]@ and @#to_json@ is a new OOP instance now. But... the cool thing is: You can package representers in gems and distribute them to your client layer as well.


h3. Using HTTP

Naturally, REST clients need to make HTTP requests to services. There are plenty of great gems helping out, namely Restfulie, HTTParty, etc. Representers provide support for HTTP, given you mix in the @HTTPVerbs@ feature module.

In order to create a new order, the frontend needs to place a POST request to the REST backend. Here's how this could happen using a representer.
...
A couple of steps happen here.

# Using the constructor a blank order document is created.
# Initial values like the client's id are passed as arguments.
# This document is <code>POST</code>ed to the given URL.
...
The client is happy! He created a real order, now he could add articles using the frontend.

h3. Using Hypermedia


Now making that system RESTful basically means

# The frontend knows one _single entry point_ URL to the REST services. This is @http://articles@.
...
Representers provide an easy DSL for defining documents and an object-oriented way for accessing and modifying.
